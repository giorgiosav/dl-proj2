    ...
    for i in range(n_runs):
        ...
        if i == plot_model:
            ...
            losses, errors = train_myNN(model, train_data, train_targets, test_data, test_targets,
                                        epochs, batch_size, eta, momentum, plots)
        else:
            losses, errors = train_myNN(model, train_data, train_targets, test_data, test_targets,
                                        epochs, batch_size, eta, momentum, False)
...
def test_pytorch_model(activation: str, eta: float, momentum: float, plots: bool, n_runs: int):
    ...
    for i in range(n_runs):
        ...
        losses, errors = train_pytorch(model, train_data, train_targets, test_data, test_targets,
...
def main(activation: str, validation: bool, pytorch: bool, plots: bool, n_runs: int):
    ...
    if len(sys.argv) == 1:
        print("\n-------------------------------------------------------")
        print("No arguments defined. Default configuration used.\n"
...
if __name__ == '__main__':
    ...
    group_models.add_argument('-relu', action='store_const', help="Use ReLU as activation function (default)",
                              dest="activation", const="relu")
    group_models.add_argument('-tanh', action='store_const', help="Use Tanh as activation function", dest="activation",
                              const="tanh")
