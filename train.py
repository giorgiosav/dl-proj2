...
def one_hot(tensor: torch.Tensor) -> torch.Tensor:
    """
    :param tensor: (torch.Tensor) input tensor of targets, of shape Nx2
    :return: (torch.Tensor) one-hot representation of input tensor
    """
...
def train_myNN(
    ...
    for e in range(epochs):
        ...
        for data, target in zip(
            ...
            model.backward(
                criterion.backward()
            )  # TODO: add support for just criterion.backward()?
            optimizer.step()
